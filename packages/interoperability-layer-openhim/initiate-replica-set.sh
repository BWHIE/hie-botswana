...
for i in $(seq 1 "$MONGO_SET_COUNT"); do
    log info "Inside the for mongo set count loop"
...
until [[ $running_instance_count -eq $MONGO_SET_COUNT ]]; do
    log info "Inside the until running instance count loop"
...
until [[ $reachable_instance_count -eq $((MONGO_SET_COUNT + 1)) ]]; do
    log info "Inside the until reachable instance count loop"
