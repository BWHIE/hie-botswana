...
# input file path
html_file_path = 'input/LAB TEST DICTIONARY_1.htm'

# Output JSON file
output_json_path = 'output/LAB_TEST_DICT.json'
...
def execute_steps(lab_test):
    ...
    the_new_dict, index = map_lab_sites(the_lines[2:], the_columns[2:], next_columns, column_indices)
    the_dict.update(the_new_dict)
    index = index + 1
    the_new_dict_one, the_index = map_texts_to_dicts(the_lines, the_columns, index, 'RESULT CODES')
    the_dict = insert_value_dict('#', the_new_dict_one, the_dict)

    the_renewed_dict, renewed_index = update_dict_variant(the_lines[the_index:], the_columns[the_index:], index,
                                                          'NOR-LO', 'NOR-HI', 'REFLEX', 'SPEC')
    the_dict = insert_value_dict('RES GRP I', the_renewed_dict, the_dict)

    the_renewed_dict_one, renewed_index_one = update_dict_variant(the_lines[the_index:], the_columns[the_index:], index,
                                                                  'REFLEX', 'SPEC', '(SHORT)', '(LONG)')
    ...
    the_component_dict, component_index = update_dict_variant(the_lines[renewed_index:], the_columns[renewed_index:],
                                                      renewed_index,
                                                      'COMPONENT', 'PROFILE','BROADCAST', 'COMPLETE')
    the_dict = insert_value_dict('COMPONENTS', the_component_dict, the_dict)

    the_lis_dict, lis_index = update_dict_by_key(the_lines[component_index:], the_columns[component_index:],
                                                 component_index, 'LIS SITE')
