...
const theConfig = JSON.parse(
  fs.readFileSync(`${__dirname}/../app-settings.json`, 'utf-8'),
);
...
export class Mediator {
  ...
  public async start(callback: any) {
    logger.info(
      'Running Service as a mediator with ' + `${__dirname}/${config}`,
    );
    ...
    try {
      ...
      registerMediator(
        theConfig.mediatorConfig.openHimAuth,
        medConfig,
        Mediator.registrationCallback(callback),
      );
    } catch (e: any) {
      logger.error(
        `Could not start Service as a Mediator!\n${JSON.stringify(e)}`,
      );
      process.exit(1);
    }
  ...
  private static registrationCallback(callback: any) {
    return (err: Error | null) => {
      if (err) {
        ...
        fetchConfig(
          theConfig.mediatorConfig.openHimAuth,
          Mediator.setupCallback(callback),
        );
      }
    };
  }
  ...
  private static async startServer(appInstance, port, callback) {
    try {
      const configEmitter = activateHeartbeat(
        theConfig.config.mediatorConfig.openHimAuth,
      );
