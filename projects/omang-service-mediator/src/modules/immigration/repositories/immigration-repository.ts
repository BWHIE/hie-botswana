...
export class ImmigrationRepository{
    ...
    private getImmigrationRecordFromRow(row: any): ImmigrationRecord {
        ...
        result.PASSP_ISSUE_DATE = row.PASSP_ISSUE_DATE ? new Date(row.PASSP_ISSUE_DATE) : null;
        result.PASSP_EXPIRY_DATE = row.PASSP_EXPIRY_DATE ? new Date(row.PASSP_EXPIRY_DATE) : null;
        ...
        result.CITIZENSHIP_NAME = row.CITIZENSHIP_NAME ? row.CITIZENSHIP_NAME.toString() : null;
        result.GENDER = row.GENDER ? row.GENDER.toString() : null;
        result.BIRTH_DATE = row.BIRTH_DATE ? new Date(row.BIRTH_DATE) : null;
        result.BIRTH_COUNTRY_CODE = row.BIRTH_COUNTRY_CODE ? row.BIRTH_COUNTRY_CODE.toString() : null;
        result.BIRTH_COUNTRY_NAME = row.BIRTH_COUNTRY_NAME ? row.BIRTH_COUNTRY_NAME.toString() : null;
        result.MARITAL_STATUS_CODE = row.MARITAL_STATUS_CODE ? row.MARITAL_STATUS_CODE.toString() : null;
        result.MARITAL_STATUS_NAME = row.MARITAL_STATUS_NAME ? row.MARITAL_STATUS_NAME.toString() : null;
        result.DATE_OF_ENTRY = row.DATE_OF_ENTRY ? new Date(row.DATE_OF_ENTRY) : null;
        result.ENTRY_PLACE_CODE = row.ENTRY_PLACE_CODE ? row.ENTRY_PLACE_CODE.toString() : null;
        result.ENTRY_PLACE_NAME = row.ENTRY_PLACE_NAME ? row.ENTRY_PLACE_NAME.toString() : null;
        result.SPOUSE_FIRST_NAME = row.SPOUSE_FIRST_NAME ? row.SPOUSE_FIRST_NAME.toString() : null;
        result.SPOUSE_SURNAME = row.SPOUSE_SURNAME ? row.SPOUSE_SURNAME.toString() : null;
    ...
    async getByLastName(lastName: string, pager: Pager): Promise<ImmigrationRecord[]> {
    ...
    async getByName(firstName: string, lastName: string, pager: Pager): Promise<ImmigrationRecord[]> {

        const filter = "UPPER(FIRST_NAME) LIKE UPPER(:firstName) AND UPPER(SURNAME) LIKE UPPER(:lastName)";
    ...
    async getByNameWithMiddleName(firstName: string, middleName: string, lastName: string, pager: Pager): Promise<ImmigrationRecord[]> {
        // First prepare the database query
        const filter = "UPPER(FIRST_NAME) LIKE UPPER(:firstName) AND UPPER(MIDDLE_NAME) LIKE UPPER(:middleName) AND UPPER(SURNAME) LIKE UPPER(:lastName)";
        const fParameter = firstName + "%";
    ...
    async getMany(passportNo: string[], pager: Pager): Promise<ImmigrationRecord[]> {
        ...
        const filter = passportNo.map((_, i) => `UPPER(PASSPORT_NO) = :ID${i}`).join(' OR ');
    ...
    async findByBirthDate(startDate: Date | null, endDate: Date | null, pager: Pager): Promise<ImmigrationRecord[]> {
    ...
    async findByEntryDate(startDate: Date | null, endDate: Date | null, pager: Pager): Promise<ImmigrationRecord[]> {
    ...
    async findByPassportExpiryDate(startDate: Date | null, endDate: Date | null, pager: Pager): Promise<ImmigrationRecord[]> {
    ...
    async findByCountry(country: string, pager: Pager): Promise<ImmigrationRecord[]> {
        country += "%";
