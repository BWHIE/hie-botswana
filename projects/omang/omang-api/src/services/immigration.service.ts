...
import { mapImmigrationRecordToFhirPatient, mapImmigrationRecordToSearchBundle, mapOmangToSearchBundle } from '../utils/fhirmapper';
...
export class ImmigrationService  extends BaseService{
  ...
  async getPatientByPassportNumber(ppn: string, pager: Pager): Promise<fhirR4.Bundle> {
  ...
  async findByBirthDate(startDate: Date, endDate: Date, pager: Pager): Promise<ImmigrationRecord[]> {
  ...
  async findByEntryDate(startDate: Date, endDate: Date, pager: Pager): Promise<ImmigrationRecord[]> {
    return this.repo.findByEntryDate(startDate, endDate, pager);
  }

  async findByPassportExpiryDate(startDate: Date, endDate: Date, pager: Pager): Promise<ImmigrationRecord[]> {
  ...
  async getByFullNameNonFHIR(firstName: string, lastName: string, pager: Pager): Promise<ImmigrationRecord[]> {
  ...
  async getByFullName(firstName: string, lastName: string, pager: Pager): Promise<fhirR4.Bundle> {
    const names = firstName.split(' ');
  ...
  async getByPassportNo(passportNo: string[], pager: Pager): Promise<ImmigrationRecord[]> {
