...
module.exports = class fhir extends dataHandler {
  ...
  getSourceLocation(bundle) {
    try {
      ...
      let sourceLocation = bundle.entry.find(
        e => e.resource.resourceType == 'Location' && e.resource.id == sourceLocationId,
      )
   ...
   *
   * This approach ensures ADT messages always have complete data, even when Location
   * resources don't have the required IPMS extensions.
   *
   * @param {Object} bundle - The FHIR bundle containing Location resources
   * @param {Object} res - The result object to populate with targetFacility data
   * @returns {Object} The result object with targetFacility populated
   */
  setIpmsLocationInformation(bundle, res) {
    try {
      let task = this.getResource(bundle, 'Task')
      const targetLocationId = task.location ? task.location.reference.split('/')[1] : null
      let targetLocation = bundle.entry.find(
        e => e.resource.resourceType == 'Location' && e.resource.id == targetLocationId,
      )
      if (targetLocation && targetLocation.resource) {
        let target = targetLocation.resource
        let provider =
          target.extension &&
          target.extension.find(e => e && e.url && e.url.includes('ipms-provider') && e.valueString)
        let patientType =
          target.extension &&
          target.extension.find(
            e => e && e.url && e.url.includes('ipms-patient-type') && e.valueString,
          )
        let patientStatus =
          target.extension &&
          target.extension.find(
            e => e && e.url && e.url.includes('ipms-patient-status') && e.valueString,
          )
        // let ipmsXlocation = target.extension.find(e => e && e.url && e.url.includes("ipms-xlocation") && e.valueString)
        let ipmsXlocation = target.identifier.find(
        ...
        res.targetFacility = {
          ...
          patientType: patientType ? patientType.valueString : IPMS_CONFIG.DEFAULT_PATIENT_TYPE, // Fallback default
  ...
  parseOrm(bundle) {
    ...
    res.facilityId = labLocation
      ? labLocation.identifier.find(
          ({ system }) => system === 'http://moh.bw.org/ext/identifier/facility-code',
        )?.value || ''
      : ''
    ...
    try {
      orderDateTime = serviceRequest.authoredOn
        ? new Date(serviceRequest.authoredOn)
        : task.authoredOn
        ? new Date(task.authoredOn)
        : orderDateTime
      res.labOrderDatetime = orderDateTime
        ? orderDateTime.getFullYear().toString() +
          (orderDateTime.getMonth() + 1).toString().padStart(2, '0') +
          orderDateTime.getDate().toString().padStart(2, '0') +
          orderDateTime.getHours().toString().padStart(2, '0') +
          orderDateTime.getMinutes().toString().padStart(2, '0') +
          orderDateTime.getSeconds().toString().padStart(2, '0')
        : ''
    } catch (e) {
      console.error(e)
    ...
    if (
      ...
      serviceRequest.code.coding.length > 0
    ) {
      let ipmsCode = serviceRequest.code.coding.find(
        e => e.system == 'https://api.openconceptlab.org/orgs/I-TECH-UW/sources/IPMSLAB/',
      )
      ...
      if (ipmsCode && ipmsCode.extension && ipmsCode.extension.length > 0) {
        let orderTypeFlag = ipmsCode.extension.find(
          e => e && e.url && e.url.includes('ipms-order-type') && e.valueString,
        )
  ...
  setPatientData(patient, res) {
    ...
    if (patient.address && patient.address.length > 0) {
      res.patientStreetAddress =
        patient.address[0].line && patient.address[0].line.length > 0
    ...
    res.patientMaritalStatus =
      patient.maritalStatus && patient.maritalStatus.coding.length > 0
    ...
    if (patient.identifier) {
      ...
      q = patient.identifier.find(
        i => i.system == 'http://moh.bw.org/ext/identifier/unknown' && i.value,
      )
